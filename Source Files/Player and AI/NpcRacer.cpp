...
NpcRacer::NpcRacer(Track& t, sf::Vector2f startPos, sf::Color color) :Racer(t, startPos, color), baseSpeed(std::rand() % 20 + 15), currentSpeed(0), m_color(color),
moveInterval(2.0f)
{
...
void NpcRacer::position(const float& dt)
{
	...
	float newY = pos.y - ( currentSpeed * 0.1f * positionScale * GameGlobals::FPS_MULTIPLIER * dt);
	...
	//position sprite
	if (trackPos < 90)
		activeSprite->setPosition(sf::Vector2f((pos.x - (distanceFromCenter() + nextOffset) * (1 - positionScale) *
			GameGlobals::FPS_MULTIPLIER* dt), pos.y));
	else
		activeSprite->setPosition(sf::Vector2f(pos.x - distanceFromCenter() * (0.02f) * GameGlobals::FPS_MULTIPLIER * dt, pos.y));
...
{
	if(!dead)
		currentSpeed = baseSpeed * Racing::Util::convertRange(trackPos, 0, 149, 0, 1) + (baseSpeed * 3) * (1 - track.getAcceleration());
	else
		currentSpeed = -baseSpeed * Racing::Util::convertRange(trackPos, 0, 149, 0, 1) + (baseSpeed * 3) *  track.getAcceleration();
}
